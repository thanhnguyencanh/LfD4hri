...
def main(args):
    ...
    for video in video_list:  # Fixed unpacking issue
        ...
        with torch.no_grad():
            ...
            index = 0
            list_for_cropped_object_link = list()
            ...
            while (num_images >= 1 and index <= num_images_for_index):
                ...
                if obj_dets is not None:
                    for i in range(num_detected_object):
                        ...
                        obj_img = crop_image(im, obj_bbox, expand_ratio=1)
                        cv2.imwrite(os.path.join(args.cropped_place_dir, f"cropped_{index}.jpg"), obj_img)
                        list_for_cropped_object_link.append(os.path.join(args.cropped_place_dir, f"cropped_{index}.jpg"))
            ...
            responses = [caption_image(image_path, prompt) for image_path in list_for_cropped_object_link[:2]]
