...
if __name__ == "__main__":
    ...
    parser.add_argument("--policy_name", default="darc")  # Policy name
    ...
    parser.add_argument("--q_weight", default=0.2, type=float)  # Range to clip target policy noise
    parser.add_argument("--reg", default=0.005, type=float)  # Range to clip target policy noise
    ...
    policy = Agent(state_dim=state_dim, action_dim=action_dim, max_action=max_action, warmup=args.warmup, lr=1e-4,
                   writer=writer, discount=args.discount, tau=args.tau, policy_noise=args.policy_noise, hidden_sizes=[512, 512],
                   noise_clip=args.noise_clip, normalizer=normalizer, chkpt_dir=os.path.join(base, args.checkpoint), q_weight=args.reg,
                    regularization_weight=args.reg)
