...
class Actor(nn.Module):
    def __init__(self, state_dim, p_action_dim=4, action_dim=7, max_action=1, lr=1e-4,
                 name='actor', chkpt_dir='ckpt/our_td3'):
        ...
        # self.l3 = nn.Linear(512 + 32, action_dim)  # Concatenate discrete embedding
    ...
    def forward(self, x):
        ...
        p_action_id = torch.argmax(p_action_logits, dim=-1, keepdim=True)  # Deterministic choice
...
class Critic(nn.Module):
    def __init__(self, state_dim, action_dim=7, p_action_dim=5, lr=1e-4,
                 name='critic', chkpt_dir='ckpt/our_td3'):
    ...
    def forward(self, x, u, p_action_id):
        p_action_id = p_action_id.view(-1)  # Flatten to (batch_size,)
...
class Agent(object):
    def __init__(self, state_dim, action_dim, p_action_dim, max_action=1, lr=1e-4, epsilon=1.0, eps_min=0.05, eps_dec=1e-5, discount=0.99, tau=0.001
                 , policy_noise=0.2, noise_clip=0.2, policy_freq=2, warmup=20000, writer=None):
        ...
        self.critic_target = Critic(state_dim=state_dim, action_dim=action_dim, p_action_dim=p_action_dim, lr=lr).to(device)
    ...
    def select_action(self, state, noise_scale=0.5, validation=False):
        ...
        if validation:
            ...
            # Adaptive noise scale: higher early, decays over time
            current_noise_scale = noise_scale * max(0.01, 1 - self.update_step / 700000) #reduce to 0.01 after 2000 episode
            if self.update_step < self.warmup:
                joint_action = torch.tensor(np.random.normal(scale=current_noise_scale, size=(self.action_dim,))).to(device)
            else:
    ...
    def learn(self, replay_buffer, batch_size=128):
        ...
        if self.update_step % self.policy_freq == 0:
            ...
            for p_a in range(self.p_action_dim):
                ...
                q1, q2, q3 = self.critic(state, joint_action, p_a_id)
        ...
        self.writer.add_scalar(
            "Q/Q3_mean", current_Q3.mean().item(), self.update_step
        )
        self.update_step += 1
