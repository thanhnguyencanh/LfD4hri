...
class ImitationLearning(gym.Env):
    ...
    def reset(self):
        ...
        self.observation['obs'] = np.concatenate((self.state,  # joint position + joint velocity
                                           np.zeros(2),  # suction state, contact state
                                           self.task_state_vector,
                                           np.zeros(7),  # previous action
                                           ))
        self.observation['state'] = np.concatenate((self.observation['obs'],
                                                    self._read_info(self.pick_object_id)[10:]))
    ...
    def step(self, action):
        ...
        self.task_state_vector += self._random_noise(self.task_state_vector, 0.005)  # randomize
        # Update next state
        self.observation['obs'] = np.concatenate((self.state,
                                           [suction_signal,
    ...
    def _read_info(self, obj_id):
        ...
        dimensions = [max_v - min_v for min_v, max_v in
                      zip(aabb_min, aabb_max)]  # object dimension: width, height, depth
